...
Pra ler secrets do Cofre (tokens, senhas, API keys), use a skill dedicada
**`intelliforce-vault`**. Lá tem `vault.py list` e `vault.py get <slug> --skill`,
o padrão de uso em scripts de outras skills, e as regras de segurança
(nunca printar valor pro user, nunca persistir, etc.).
