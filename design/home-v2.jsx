// IntelliForce Home v2 — Departments as floors, agents as tiles
...
// ===== Org structure =====
// Departments → teams → roles → agent count by status
const ORG = [
  {
    id: "finance",
    name: "Finance",
...
// Agent tile — reuses the brand notched-square shape
function AgentTile({ status = "active", size = 14, skill }) {
  const colors = {
    active: "var(--success)",
    idle: "var(--warning)",
  ...
  return (
    <div
      ...
      style={{ width: size, height: size, color: colors[status] }}
      title={`${skill || "agent"} · ${status}`}
    >
...
// Generate a deterministic-ish array of agents for visualization
    ...
    <section className={`dept dept--${dept.health}`}>
      ...
      <div className="dept-left">
        <div className="dept-id">
          ...
          <a className="dept-edit" href={`Department setup.html?id=${dept.id}`} title="Edit department">
...
// Sparkline timeline component
function Timeline({ data, health }) {
