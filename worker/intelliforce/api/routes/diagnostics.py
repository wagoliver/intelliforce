...
# Heurística pra estimar tokens a partir de bytes de texto latino (~4 bytes/token).
# Usada no enriquecimento do check de OpenCode pra alertar quando o pior agent
# se aproxima do context length configurado no LM Studio.
_BYTES_PER_TOKEN = 4
# Fração do contexto que deve sobrar pra resposta + histórico antes de o prompt
# ser considerado apertado.
...
def _estimate_opencode_prompt_size(settings: Settings) -> dict | None:
    """Estima o tamanho do pior system prompt (agent + todas as skills).

    Retorna None se o diretório `.opencode` não existe. Caso contrário, devolve
    {worst_agent, worst_tokens, agents_count, skills_count, ctx_length}.

    Pessimista por design: OpenCode faz progressive disclosure (só metadado da
    skill entra no prompt inicial), mas observamos n_keep do LM Studio bater
    com o tamanho total — assumimos pior caso pra alertar cedo.
    """
    ...
    if skills_dir.exists():
        for skill_md in skills_dir.glob("*/SKILL.md"):
            skills_total_bytes += skill_md.stat().st_size
            skills_count += 1
    ...
    if agents_dir.exists():
        for agent_md in agents_dir.glob("*.md"):
            agents_count += 1
            total_bytes = agent_md.stat().st_size + skills_total_bytes
            est_tokens = total_bytes // _BYTES_PER_TOKEN
            if est_tokens > worst_tokens:
                worst_tokens = est_tokens
                worst_agent = agent_md.stem
    ...
    return {
        ...
        "ctx_length": settings.lmstudio_context_length,
    }
...
async def check_opencode(settings: Settings) -> DiagnosticComponent:
    ...
    prompt_size = await asyncio.to_thread(_estimate_opencode_prompt_size, settings)
    if prompt_size and prompt_size["agents_count"] > 0:
        worst_tokens = prompt_size["worst_tokens"]
        ctx = prompt_size["ctx_length"]
        worst_agent = prompt_size["worst_agent"]
        meta.append(
            f"{prompt_size['agents_count']} agents, {prompt_size['skills_count']} skills"
        )
        if worst_tokens > ctx:
            component_status = "err"
            message = (
                f"Pior agent '{worst_agent}' estima ~{worst_tokens} tokens, "
                f"acima do contexto configurado ({ctx})."
            )
            causes = [
                "Skills ativas excedem context length do modelo",
                f"Recarregue o modelo com context length ≥ {max(32768, worst_tokens * 2)}",
            ]
        elif worst_tokens > ctx * _CTX_HEADROOM_FRACTION:
            pct = round(worst_tokens / ctx * 100)
            component_status = "warn"
            message = (
                f"Pior agent '{worst_agent}' usa ~{worst_tokens} tokens "
                f"({pct}% de {ctx}) — margem apertada."
            )
            causes = [
                "Sobra pouco contexto pra resposta e histórico",
                "Aumente context length ou reduza skills ativas",
            ]
        else:
            meta.append(f"pior prompt: ~{worst_tokens} tok ({worst_agent})")
...
async def check_admins(settings: Settings) -> DiagnosticComponent:
    """Garante que existe ao menos 1 admin humano ativo — fail-safe contra
    lockout da plataforma após desativações ou criação acidental só de
    service accounts."""
    last_check = _now()
    description = (
        "Contas humanas com papel admin no IntelliForce. Sem nenhuma, ninguém "
        "consegue operar a plataforma."
    )
    ...
    try:
        ...
        log.exception("diagnostics.admins.query_failed")
        return DiagnosticComponent(
            id="admins",
            name="Admins humanos",
            description=description,
            status="err",
            message=f"Falha ao consultar usuários: {type(exc).__name__}",
            causes=[str(exc)[:200]] if str(exc) else [],
            last_check=last_check,
        )
    ...
    if count == 0:
        return DiagnosticComponent(
            ...
            description=description,
            status="err",
            metric=Metric(value="0", label="ativos"),
            message="Nenhum admin humano ativo.",
            causes=[
    ...
    return DiagnosticComponent(
        ...
        description=description,
        status="ok",
        metric=Metric(value=str(count), label="ativos"),
        meta=[f"{count} admin{plural} humano{plural} ativo{plural}"],
        last_check=last_check,
    )
...
COMPONENT_ORDER: list[ComponentId] = [
    "llm", "opencode", "env", "worker", "redis", "postgres", "clickhouse", "admins",
]
